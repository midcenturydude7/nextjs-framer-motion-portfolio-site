        ...
        {navIcons.map(({ icon: Icon, url, id, label }) => (
          <Link key={id} href={url} target="_noblank">
            <button
              ...
              tabIndex={0}
              className={`relative h-[2.313rem] w-[2.188rem] cursor-pointer rounded-full bg-slate-900 px-5 py-5 text-[1.25rem] text-[#E8E8FD]/50 outline-none transition-colors hover:text-purple-200/90 hover:duration-1000 group-hover:duration-1000`}
            >
              ...
              <AnimatePresence>
                {circle === label ? (
                  <motion.div
                    className="absolute inset-[0.5px] z-0 h-[100%] w-[100%] rounded-full bg-[rgba(57,63,73,0.75)]"
                    transition={{
                      layout: {
                        duration: 0.4,
                        ease: "easeOut",
